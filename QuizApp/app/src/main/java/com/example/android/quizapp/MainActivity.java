...
public class MainActivity extends AppCompatActivity {
    ...
    public void submitAnswers(View v) {
       ...
       else if (continentRadioGroup.getCheckedRadioButtonId() == -1) {

            Toast.makeText(this, "Select one answer on question 1", Toast.LENGTH_SHORT).show();
        }

        else if (!whiteColor.isChecked() && !greenColor.isChecked() && !redColor.isChecked() && !grayColor.isChecked() && !pinkColor.isChecked()) {

            Toast.makeText(this, "Select correct answers on question 2", Toast.LENGTH_SHORT).show();

        }

        else if (TextUtils.isEmpty(question3.getText())) {

            question3.setError("Answer is required!");

        }

        else if (colorsRadioGroup.getCheckedRadioButtonId() == -1) {

            Toast.makeText(this, "Select one answer on question 4", Toast.LENGTH_SHORT).show();
        }

        else if (TextUtils.isEmpty(question5.getText())) {

            question5.setError("Answer is required!");

        }

        else  if (billgatesRadioGroup.getCheckedRadioButtonId() == -1) {
